...
func (r *Runner) buildPrompt(params SpawnParams) string {
	...
	sb.WriteString(fmt.Sprintf("- To talk privately to your user: send_message(to=%q, text=\"your message\") — only they will see it.\n", params.Sender))
	sb.WriteString("  If you need to ask them something, use this. They will reply in the chat box.\n")
	sb.WriteString("  After asking, call get_messages to poll for their reply before continuing.\n")
	sb.WriteString("- For announcements visible to everyone in the room: send_message(text=\"...\") with no `to`.\n")
